Public Class RandIntBetweenNumExprNode
    ...
    Sub New(ByVal min As ParseTreeNode, ByVal max As ParseTreeNode, Optional ByVal seed As ParseTreeNode = Nothing)
        ...
        If IsNothing(_min) Then
            ...
            _min = New ErrorNode(TypeEnum.Int, "Empty", "")
        Else
            If (_min.Type And TypeEnum.Int) = 0 Then _type = TypeEnum.ERR : _ParseError = "The min value in the randbetween function did not produce an integer output"
        End If
        _max = max
        If IsNothing(_max) Then
            _type = TypeEnum.ERR : _ParseError = "The max value in the randbetween function was not defined"
            _max = New ErrorNode(TypeEnum.Int, "Empty", "")
        Else
            If (_max.Type And TypeEnum.Int) = 0 Then _type = TypeEnum.ERR : _ParseError = "The max value in the randbetween function did not produce an integer output"
        End If
        If _ParseError <> "" Then _ParseErrors.Add(_ParseError)
    End Sub
    Public Overrides Function Evaluate() As ParseTreeNodeResult
        Dim min As Integer
        Integer.TryParse(_min.Evaluate.GetResult.ToString, min)
        Dim max As Integer
        Integer.TryParse(_max.Evaluate.GetResult.ToString, max)
        Return New ParseTreeNodeResult(_RAND.Next(min, max), TypeEnum.Num)
    End Function
    Public Overrides Function EvaluateSingle() As ParseTreeNodeResult
        ...
        Dim max As Integer
        Integer.TryParse(_max.Evaluate.GetResult.ToString, max)
        Return New ParseTreeNodeResult(CSng(_RAND.Next(min, max)))
    End Function
    Public Overrides Function ExpressionToString() As String
        If IsNothing(_seed) Then
            Return "RANDINTBETWEEN(" & _min.ExpressionToString & "," & _max.ExpressionToString & ")"
        Else
            Return "RANDINTBETWEEN(" & _min.ExpressionToString & "," & _max.ExpressionToString & "," & _seed.ExpressionToString & ")"
        End If
