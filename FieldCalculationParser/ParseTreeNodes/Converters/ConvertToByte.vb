Public Class ConvertToByte
    ...
    Public Overrides Function Evaluate() As ParseTreeNodeResult
        Dim ret As Byte
        If Byte.TryParse(_node.Evaluate.GetResult.ToString, ret) Then
            Return New ParseTreeNodeResult(ret, TypeEnum.byt)
        Else
            _ComputeErrors.Add("The value: " & _node.Evaluate.GetResult.ToString & ", cannot be converted to Byte, so the value 0 was used on row " & RowOrCellNum)
            Return New ParseTreeNodeResult(0, TypeEnum.byt)
    ...
    Public Overrides Function ExpressionToString() As String
        Return "Byte(" & _node.ExpressionToString & ")"
    End Function
